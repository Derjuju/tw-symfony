...
/**
 * Description of AddressTWListener
 *
 * @author pinacolada
 */

class AddressTWListener
{
    
    private $em;
    
    public function prePersist(AddressTW $address, LifecycleEventArgs $event)
    {
        $this->em = $event->getEntityManager();        
        $address = $this->geolocaliseAddress($address, true);        
    }
    
    public function preUpdate(AddressTW $address, LifecycleEventArgs $event)
    {
        $this->em = $event->getEntityManager();        
        $address = $this->geolocaliseAddress($address, false);
    }
    
    public function postUpdate(AddressTW $address,LifecycleEventArgs $event)
    {
        $this->em = $event->getEntityManager(); 
    }
    
    private function geolocaliseAddress(AddressTW $address, $force){
